import { ArrowLeft, Phone, Video, Mic, Image, Heart, Users, Edit2, Trash2 } from "lucide-react";
    ...
    <div className={cn("flex flex-col h-full", bgColor)}>
      <div className={cn("px-3 py-2 flex items-center shadow-sm z-10", bgColor)}>
        ...
        <div className="ml-2 flex items-center gap-3 flex-1">
          ...
          <div>
            <p className={cn("font-semibold text-[15px] leading-tight", textColor)}>
            ...
            {appearance.showStatus && (
              <p className={cn("text-[11px]", subtextColor)}>
                {appearance.statusText || (displayPerson?.isOnline ? 'Active now' : 'Active 2h ago')}
              </p>
            )}
          </div>
        </div>
        <div className="flex gap-5 px-2">
          ...
          <Video className={cn("w-6 h-6", textColor)} />
        </div>
      </div>
      <div className={cn("flex-1 overflow-y-auto px-4 py-2 space-y-1 mt-1", bgColor)} style={getWallpaperStyle(appearance)}>
      ...
      <div className={cn("p-3", bgColor)}>
        <div className={cn("flex items-center gap-3 rounded-full border px-4 py-2", appearance.darkMode ? "border-[#363636]" : "border-[#dbdbdb]")}>
          ...
          <div className="flex gap-4">
            <Mic className={cn("w-5 h-5", textColor)} />
            <Image className={cn("w-5 h-5", textColor)} />
            <Heart className={cn("w-5 h-5", textColor)} />
          </div>
        </div>
      </div>
    </div>
