import { ArrowLeft, Search, MoreVertical, Smile, Plus, Mic, Users } from "lucide-react";
    ...
    <div className={cn("flex flex-col h-full", bgClass)}>
      <div className={cn("px-3 py-2 flex items-center border-b", headerBg, appearance.darkMode ? "border-transparent" : "border-[#c8c8cc]")}>
        ...
        <div className="ml-3 flex-1">
          ...
          {appearance.showStatus && <p className={cn("text-xs", iconColor)}>{chatType === 'group' ? `${people.length} members` : displayPerson?.isOnline ? 'online' : 'last seen recently'}</p>}
        </div>
        <div className="flex items-center gap-2"><Search className={cn("w-5 h-5", iconColor)} /><MoreVertical className={cn("w-5 h-5", iconColor)} /></div>
      </div>
      <div className="flex-1 overflow-y-auto p-3 space-y-1" style={{
        ...
        {appearance.showTimestamps && (
          <div className="flex justify-center mb-2"><span className={cn("text-xs px-3 py-1 rounded-full", dateBadgeBg)}>{format(today, 'MMMM d')}</span></div>
        )}
        {messages.map((message) => (
          <div key={message.id} className={cn("flex flex-col", message.isOwn ? "items-end" : "items-start")}>
            <div className={cn("max-w-[85%] px-3 py-1.5 rounded-xl", message.isOwn ? cn(ownBubble, "rounded-tr-[4px]") : cn(otherBubble, "rounded-tl-[4px]"))} style={{ wordBreak: 'break-word' }}>
