...
export const TwitterComments: React.FC<TwitterCommentsProps> = ({ state }) => {
    ...
    const renderComment = (comment: Comment, depth = 0) => {
        ...
        return (
            <div key={comment.id} className={cn("flex gap-3 border-b px-4 py-3", depth > 0 && "ml-8 border-l", isDark ? "border-zinc-800" : "border-gray-100")}>
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1 text-[15px]">
                        <span className={cn("truncate font-bold", isDark ? "text-white" : "text-black")}>{profile.name}</span>
                        {profile.verified && <VerifiedBadge platform="twitter" className="w-[18px] h-[18px]" />}
                        <span className={cn("truncate", isDark ? "text-zinc-500" : "text-gray-500")}>@{profile.handle}</span>
                        <span className={isDark ? "text-zinc-500" : "text-gray-500"}>·</span>
                        <span className={isDark ? "text-zinc-500" : "text-gray-500"}>{comment.timeAgo}</span>
                    </div>

                    <div className={cn("mt-0.5 whitespace-pre-wrap text-[15px] leading-normal", isDark ? "text-white" : "text-black")}>
                        {comment.text}
                    </div>

                    <div className="flex items-center justify-between mt-3 max-w-[350px]">
                        <div className={cn("group flex cursor-pointer items-center gap-2 hover:text-blue-500", isDark ? "text-zinc-500" : "text-gray-500")}>
                            <MessageCircle className="w-[18px] h-[18px] group-hover:bg-blue-500/10 rounded-full p-[-2px]" />
                        </div>
                        <div className={cn("group flex cursor-pointer items-center gap-2 hover:text-green-500", isDark ? "text-zinc-500" : "text-gray-500")}>
                            <Repeat2 className="w-[18px] h-[18px]" />
                        </div>
                        <div className={cn("group flex cursor-pointer items-center gap-2 hover:text-pink-500", isDark ? "text-zinc-500" : "text-gray-500")}>
