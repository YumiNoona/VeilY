...
export const supabase = {
  auth: {
    ...
    signInWithPassword: async (..._args: unknown[]) => ({ data: { user: mockUser, session: null }, error: null }),
    signInWithOtp: async (..._args: unknown[]) => ({ data: null, error: null }),
    signUp: async (..._args: unknown[]) => ({ data: { user: mockUser, session: null }, error: null }),
    verifyOtp: async (..._args: unknown[]) => ({ data: { user: mockUser, session: null }, error: null }),
    signOut: async () => { noop(); return { error: null }; },
    updateUser: async (attrs: any) => ({ data: { user: { ...mockUser, ...attrs } }, error: null }),
    onAuthStateChange: () => ({ data: { subscription: { unsubscribe: noop } } }),
    resend: async (..._args: unknown[]) => ({ data: null, error: null }),
